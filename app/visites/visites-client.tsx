          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {v.heure_visite && <p className="text-xs text-gray-400">{v.heure_visite.slice(0, 5)}</p>}
            ...
            <div className="flex items-center gap-3">
              <span className={cn('text-xs px-2.5 py-1 rounded-full font-medium', STATUT_COLORS[v.statut] ?? 'bg-gray-100 text-gray-600')}>
                {STATUT_LABELS[v.statut] ?? v.statut}
              </span>
              {v.statut === 'confirmee' && (
                <>
                  <Button size="sm" className="bg-green-600 hover:bg-green-700" disabled={loading === v.id} onClick={() => handleRealise(v.id)}>
                    <CheckCircle className="w-4 h-4 mr-1" /> Réalisée
                  </Button>
      ...
      {active.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-blue-700 flex items-center gap-2">
            ...
            Confirmées ({active.length})
          </h2>
          {active.map(v => <VisiteCard key={v.id} v={v} />)}
        </div>
      )}
